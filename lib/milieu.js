...
class Milieu {
  constructor (applicationName, defaults, opts) {
    ...
    global.__testArgv__ && (opts.argv = global.__testArgv__)
    global.__testEnv__ && (opts.env = global.__testEnv__)
    global.__testPlatform__ && (opts.platform = global.__testPlatform__)
    global.__testCwd__ && (opts.cwd = global.__testCwd__)
  ...
  set (configUpdate) {
    const config = this.__$.config

    deepExtend(config, configUpdate)

    for (const prop of this.__$.exposedProps) {
  ...
  _compileConfig () {
    ...
    for (const prop in config) {
      if (!this[prop]) {
        this[prop] = config[prop]
        this.__$.exposedProps.push(prop)
      }
    }
  }
  ...
  _collectConfigPaths () {
    ...
    if (!isWin) {
      configPaths.push(
        // /etc/apprc
        path.join(`/etc/${this.__$.applicationName}rc`),
        // /etc/app/config
        path.join(`/etc/${this.__$.applicationName}/config`),
        // /usr/local/etc/apprc
        path.join(`/usr/local/etc/${this.__$.applicationName}rc`),
        // /usr/local/etc/app/config
        path.join(`/usr/local/etc/${this.__$.applicationName}/config`)
      )
    }
  ...
  _convertEnvKeyToPath (key) {
    key = key
      ...
      .toLowerCase()
      .replace(/___([\w])/g, (_, char) => '.' + char.toUpperCase())
      .replace(/__/g, '.')
  ...
  _convertFlagToPath (key) {
    key = key
      .replace(/_{3}|-{3}([\w])/g, (_, char) => '.' + char.toUpperCase())
      .replace(/_{2}|-{2}/g, '.')
      .replace(/[_-]([\w])/g, (_, char) => char.toUpperCase())
    return key
  }
